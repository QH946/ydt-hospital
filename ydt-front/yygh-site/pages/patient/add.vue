  ...
  data() {
    return {
      ...
      validateRules: {
        ...
        sex: [{ required: true, trigger: "blur", message: "必须输入" }],
        certificatesNo: [
          { required: true, trigger: "blur", message: "必须输入" },
        ],
    ...
    if (this.$route.query.id) {
      setTimeout(() => {
        this.$refs.selectedShow.presentText =
          ...
          "/" +
          this.patient.param.districtString; //"北京市/市辖区/西城区";// 首次手动复制 // this.$refs.selectedShow.value = '110000/110100/110102';
      }, 1000);
    }
  },
  ...
  methods: {
    ...
    fetchDataById(id) {
      patientApi.getById(id).then((response) => {
        this.patient = response.data; //添加默认值

      ...
      <div class="container">
        ...
        <div class="operate-view" v-if="dialogAtrr.showLoginType === 'weixin'">
          ...
          <div class="wrapper wechat" style="height: 400px">
            ...
                        
            <div class="bottom wechat" style="margin-top: -50px">
                            
      ...
      <div class="right-wrapper">
        ...
                  <el-dropdown v-if="name != ''" @command="loginMenu">
          ...
          >
            ...
            >
                          <el-dropdown-item command="/patient"
...
export default {
  data() {
    return {
      ...
      dialogUserFormVisible: false, // 弹出层相关属性
  ...
  mounted() {
    ...
    document.body.appendChild(script); // 微信登录回调处理
  ...
  methods: {
    ...
    btnClick() {
      ...
      if (this.dialogAtrr.loginBtn == "获取验证码") {
        this.userInfo.phone = this.dialogAtrr.inputValue; // 获取验证码
    ...
    showLogin() {
      this.dialogUserFormVisible = true; // 初始化登录层相关参数
    ...
    login() {
      ...
      userInfoApi
        .login(this.userInfo)
        .then((response) => {
          console.log(response.data); // 登录成功 设置cookie
    ...
    getCodeFun() {
      ...
      if (!this.dialogAtrr.sending) return; // 发送短信验证码
      ...
      smsApi
        ...
        .catch((e) => {
          this.$message.error("发送失败，重新发送"); // 发送失败，回到重新获取验证码界面
          this.showLogin();
        });
    },

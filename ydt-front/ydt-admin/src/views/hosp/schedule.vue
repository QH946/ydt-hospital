        ...
                        <el-row style="margin-top: 20px">
          ...
          >
            ...
                                    <el-table-column label="职称" width="150">
                                          <template slot-scope="scope"
                >{{ scope.row.title }} | {{ scope.row.docname }}</template
              >
                                      </el-table-column
            >
                                    <el-table-column
...
export default {
  ...
  created() {
    if (this.$route.params && this.$route.params.hoscode) {
      this.hoscode = this.$route.params.hoscode;
      this.workDate = this.getCurDate();
      this.fetchData();
    } else {
      this.$message.error("路由有误");
    }
  },
  methods: {
    //获取排班详情
    getDetail() {
      scheduleApi
        ...
        .then((res) => {
          //输出每次选中第几个
          console.log("第" + this.activeIndex + "页");
          // console.log(res);
          if (res.code == 200) {
      ...
      hospApi
        ...
        .then((res) => {
          ...
          if (res.code == 200) {
            ...
            //   this.getDetail(this.hoscode, this.depcode, this.workDate);
            // }
          }
        })
        .catch((e) => {
